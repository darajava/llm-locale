...
const translateFile = async (sourceFile, destFile, sourceLanguage, destLanguage, openAiKey) => {
    let source, dest, destCopy;
    ...
    const getNeededKeys = () => {
        ...
        if (missingSource.length > 0) {
            console.error();
            console.error(`ERROR: Missing keys in source translation file: ${missingSource.join(", ")}`);
            console.error(`ERROR: The dest file should have no keys that are not in the main file`);
            console.error();
            process.exit(1);
        }
        return missingDest;
    ...
    try {
        ...
        if (finalCheck.length > 0) {
            console.error();
            console.error(`ERROR: there are still missing keys in the dest file: ${finalCheck.join()}, reverting changes.`);
            console.error();
